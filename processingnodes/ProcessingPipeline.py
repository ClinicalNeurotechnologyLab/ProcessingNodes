...
from .DataProcessor import DataProcessor, check_data_dimensions, T_Data, T_Timestamps
from . import nodes as ProcessingNodes
...
class ProcessingPipeline(DataProcessor):
    def __init__(self, in_channel_labels: Union[List[str], int], nodes: Union[List[dict], List[DataProcessor]], in_feature_dims: List[int] = None, **settings):
        ...
        if all([isinstance(node, dict) for node in nodes]):
            ...
            for node_settings in nodes:
                ...
                assert type(node_settings) is dict

                # skip adding nodes that have a setting 'skip' set to True
                if node_settings.get("skip", False):
                    continue
                ...
                try:
                    ...
                    node: DataProcessor = getattr(ProcessingNodes, node_settings["type"])(in_channel_labels=intermediate_channel_labels, in_feature_dims=intermediate_feature_dims, **node_settings)
                    self.nodes.append(node)
                except TypeError as e:
                    raise ProcessingPipelineException(f"An error occurred when initializing a node of type {node_settings['type']} with in_feature_dims={intermediate_feature_dims} and settings {node_settings}:\n{e}")
            ...
            self._nodes_init: List[dict] = nodes # type: ignore
            ...
            # copy all nodes to self.nodes that do not have a 'skip' setting set to True
            self.nodes: List[DataProcessor] = [n for n in nodes if 'skip' not in n.settings or not n.settings['skip']] # type: ignore
            ...
            self._nodes_init: List[dict] = [node.settings for node in self.nodes] # type: ignore
    ...
    def process_chunks(self, data: np.ndarray, timestamps=None, chunk_size: int = 10, clean_none: bool = True, *args, **kwargs) -> Tuple[list, list]:
        ...
        for i_chunk in range(n_chunks):
            ...
            chunk: np.ndarray = data[..., start_sample:end_sample]
            chunk_timestamps = timestamps[start_sample:end_sample] if include_timestamps else None # type: ignore
    ...
    def get_class_for_value(self, values):
        for node in self.nodes[::-1]:
            if hasattr(node, 'get_class_for_value'):
                return node.get_class_for_value(values) # type: ignore
