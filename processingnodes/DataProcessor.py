...
T_Timestamps = Union[Collection[float], float, None]
...
class DataProcessor:
    ...
    def __init__(self, in_channel_labels: List[str], out_channel_labels: Optional[List[str]] = None, in_feature_dims: Optional[List[int]] = None, **settings):
        self.in_channel_labels = in_channel_labels

        self._settings = settings

        self.out_channel_labels: List[str] = []
        if out_channel_labels is None:
            self.out_channel_labels = self.generate_out_channel_labels()
        else: 
            self.out_channel_labels = out_channel_labels

        self.in_feature_dims: Optional[List[int]] = in_feature_dims
    ...
    def train(self, data: T_Data, labels: np.ndarray, timestamps=None, *args: Any, **kwargs: Any) -> Union[T_Data, Tuple[T_Data]]:
    ...
    def get_settings(
            self, 
            in_channel_labels: bool = True, 
            out_channel_labels: bool = True, 
            in_feature_dims: bool = True, 
            out_feature_dims: bool = True, 
            *args, 
            **kwargs
        ) -> T_Settings:
        """
