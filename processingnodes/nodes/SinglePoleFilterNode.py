...
class SinglePoleFilterNode(ProcessingNode):
    def __init__(self, in_channel_labels: List[str], time_const: float, sfreq: float, **settings):
        ...
        self._decay_factor: float = 0
        self._y: np.ndarray = np.array(0.0, dtype=np.float32) 
        self._init_filter()
        self.clear()
    ...
    def train(self, data: T_Data, labels: np.ndarray, timestamps=None, *args: Any, **kwargs: Any) -> Union[T_Data, List[T_Data]]:

        data_out, timestamps_out = self.process(data, timestamps)
        return data_out, labels, timestamps_out

    def clear(self, *args, **kwargs):
        self._y = np.array(0.0, dtype=np.float32)
